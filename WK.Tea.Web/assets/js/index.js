...
layui.extend({
  ...
  ,renderPage = function(){
      ...
      console.log(path);
    //默认读取主页
    if (!path.length || path[0] == "") path = ['default'];
    ...
    */
    ...
    view().render(path.join('/')).then(function(res){
      ...
      var title = router.search.title || res.title;
      var pageHeaderObj = $('.layui-header .header-title').empty();
      if (title && pageHeaderObj.length > 0) {
          //pageHeaderObj.append($('<a href="javascript:;" layadmin-event="back"><i class="layui-icon layui-icon-left"></i></a>'));
          pageHeaderObj.text(decodeURIComponent(title));
      }
    }).done(function(){
  ...
  ,entryPage = function(fn){
    ...
    layui.each(setter.indPage, function (index, item) {
        var temp = pathURL.indexOf("?") > 0 ? pathURL.substr(0, pathURL.indexOf("?")) : pathURL;
